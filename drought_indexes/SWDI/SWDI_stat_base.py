...
from geotiff import *
from create_SWDI_daily import create_SWDI_daily
        ...
        if oDate.strftime("%Y%m")  in dateListMonths:
            ...
            for m in range(monthCount):
                ...
                for date in dateList:
                    ...
                    if  year+month in date:

                        dateInMonth.append(date)

            if len(dateInMonth)>0:

                calculate_monthly_average(indexFold, snowFold, snowPrefix, dateInMonth, year, month, indexPrefix, monthCount)
...
def main(argv):
    ...
    with open('SWDI_config.json') as jf:
        params = json.load(jf)
        ...
        #create_SWDI_daily (soilmoistureParent, sDateFrom, sDateTo, [var.replace("-", "_")])
