...
def main (argv):
    ...
    with open(file_json) as jf:
        ...
        compute_combined (oDateFrom,oDateTo,products_folder,INDEX,accumulation,threshold)

    print('\n' + str(datetime.datetime.now()) + ' - Process ended.\n')



def compute_combined (oDateFrom,oDateTo,products_folder,INDEX,accumulation,threshold):
    ...
    while (oDateFrom <= oDateTo):
        ...
        for i in range(0, len(INDEX)):
            ...
            if INDEX[i]=="SPI":
                ...
                prefix= INDEX[i] + accumulation[i] + "-PERSIANN-MODIS_"
                 ...
                 prefix= INDEX[i] + accumulation[i]+ "-SMAP_"
            elif INDEX[i]=="SWDI":
                 prefix= INDEX[i] + accumulation[i]+ "-SMAP_"
            ...
            descr = indexes+"-"+INDEX[i]+accumulation[i]+"tr"+str(threshold[i])
            # PLOT combined
            """
            fig, ax = plt.subplots()
        ...
        combined_file = os.path.join(combined_dir,"Combined"+indexes+"_"+oDateFrom.strftime("%Y%m")+".tif")
