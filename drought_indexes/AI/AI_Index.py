...
def calculate_result(fileName, statsFileName, destFold, year, month, monthCount, outIndex):
    ...
    newFileName = os.path.join(dest_dir, "PrecPETratio-{:02d}_".format(monthCount)+year+month+".tif"
                               .format(monthCount))
    ...
    AI = stat.norm.ppf(probVal, loc=0, scale=1)
   ...
   # ESI[data==1] = np.nan
    writeGeotiffSingleBand (outFileName, geoTrans, geoproj,AI, nodata=np.nan, BandName="Standardized_SPEI")
...
def main(argv):
    ...
    with open('AI_config.json') as jf:
        ...
        PrecPETratioPrefix = params["Prec_PET_ratio_prefix"]
        ...
        while (oDateFrom <= oDateTo):
            ...
            for monthCount in aggMonths:
                fileName = os.path.join(indexFold,PrecPETratioPrefix+"-Monthly-Files", "{:d}-Month-Files".format(monthCount),
                                        PrecPETratioPrefix + "_" + oDateFrom.strftime("%Y%m")+".tif")
