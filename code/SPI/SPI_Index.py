...
from SPI_stat_base import convert_to_month_average_data, get_all_dates
...
def calculate_result(fileName, statsFileName, destFold, year, month, monthCount, outIndex,PrecNameIndex,maskFile):
    ...
    probVal = distr.gam.cdf(data, a=shape, loc=loc, scale=scale)
        ...
        while (oDateFrom <= oDateTo):
            ...
            for monthCount in aggMonths:
                ...
                if not os.path.exists(fileName):
                    ...
                    dateList.sort()
                    destFold = os.path.join(monthlyFolder,"{:d}-Month-Files".format(monthCount))

                    convert_to_month_average_data(PrecFold, PrecNameIndex, dateList, destFold, int(month), int(year), monthCount, maskFile)
